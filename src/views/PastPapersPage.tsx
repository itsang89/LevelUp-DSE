...
import { estimateDseLevel, hasSubjectCutoffData, resolveCutoffSubjectKey } from "../utils/dseLevelEstimator";
...
function PaperMatrix({ subject, attempts, availablePaperLabels, cutoffData }: PaperMatrixProps) {
  const { matrix, minYear, maxYear, stats } = useMemo(() => {
    ...
    let currentMinYear = currentMaxYear - 10;
    ...
    if (cutoffData && cutoffData[cutoffKey]) {
      ...
      const cutoffYears = Object.keys(subjectCutoffs).map(Number).filter(y => !isNaN(y));
    ...
    for (let year = currentMaxYear; year >= currentMinYear; year--) {
      grid[year] = {};
      for (const label of availablePaperLabels) grid[year][label] = [];
    }
    ...
    for (let year = currentMaxYear; year >= currentMinYear; year--) {
      for (const label of availablePaperLabels) {
        if (grid[year][label].length > 0) completedSlots++;
      }
    }

    const completionPercentage = totalSlots > 0 ? Math.round((completedSlots / totalSlots) * 100) : 0;
    return { matrix: grid, minYear: currentMinYear, maxYear: currentMaxYear, stats: { totalSlots, completedSlots, completionPercentage } };
  }, [attempts, subject, availablePaperLabels, cutoffData]);

  if (availablePaperLabels.length === 0) return null;
